...
contract KYCPolyMath is usingOraclize {
    ...
    */
    ...
    event LOG_CheckUserKYC(bool KYC_status);
    ...
    function applyForKYC (string _first, string _last, string _birthDate, string _citizenship) payable {
        ...
        //this allows for only applying to KYC once per address. but is this needed? i dont think so right now.
      //  if (userKYCInfoArray.length >= 1 && (KYC_ID[msg.sender] !=0 || userKYCInfoArray[0].ethereumPubKey == msg.sender) revert();
...
/**********************GETTER FUNCTIONS**************************************/
    function getter_User_Applications () external returns (uint) {
        uint usersAppliedForKYC;
        usersAppliedForKYC = userKYCInfoArray.length;
        return usersAppliedForKYC;
    }

    function getter_User_Array_Info (uint userID) external returns (bool){
        bool boolApproved = userKYCInfoArray[userID].kycApproved;
        return boolApproved;
    }
    
    function verifyFromOutside  (address checkIfUserIsApproved) external returns (bool) {
        
       bool isUserApproved = KYCVerification[checkIfUserIsApproved];
       LOG_CheckUserKYC(isUserApproved);
